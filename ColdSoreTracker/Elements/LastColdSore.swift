    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            if (numSores == 0){
                //Theres no Sores
                ZStack{
                    Rectangle().frame(height:170).cornerRadius(15).padding(.horizontal).shadow(radius: /*@START_MENU_TOKEN@*/10/*@END_MENU_TOKEN@*/).foregroundColor(.white)
                ...
                ZStack{
                    ...
                    VStack(alignment: .leading){
                        ...
                        Text("Reason: \(reason) ").padding(.top, -2)
                            .font(.body)
                            .multilineTextAlignment(.leading).foregroundColor(Color(hue: 1.0, saturation: 0.0, brightness: 0.0, opacity: 0.4))
