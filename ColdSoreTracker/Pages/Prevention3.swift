...
struct Prevention3: View {
    var body: some View {
        VStack(alignment: .leading){
            ...
            HStack(alignment: .top){
                Circle().frame(width: 5, height: 5).foregroundColor(.black).opacity(0.6).padding(.top, 6)
                Text("When a sore is active, avoid touching it and wash your hands often. This can help keep you from spreading the virus to your eyes or genital area or to other people." ).font(.subheadline).foregroundColor(.black).opacity(0.6)
            }.padding(.top)
