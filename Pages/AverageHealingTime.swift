...
struct AverageHealingTime: View {
    ...
    var body: some View {
        ...
        VStack(alignment: .leading){
            Text("Average Healing Time (Cause)").font(.headline).fontWeight(.medium).foregroundColor(.black).opacity(0.6).multilineTextAlignment(.leading).padding(.top, 10.0).minimumScaleFactor(0.5)
            if(averageduration == 0 || averageduration.isNaN){//Change this to 0 after testing done
                Text("Add a duration to either a new / one of your existing cold sores to track average healing times.").font(.subheadline).foregroundColor(.black).opacity(0.6).padding(.top)
                Text("Simply navigate to the calendar, long press a cold sore, select edit, and add a duration, easy!").font(.footnote).foregroundColor(.black).opacity(0.6).padding(.top)
                ...
                ZStack{
                    //Show average data here
                        ...
                        Text("Duration (days)").font(.caption).rotationEffect(.degrees(-90)).position(x: 8, y: CGFloat(85 + 13*numDifferentSores))
